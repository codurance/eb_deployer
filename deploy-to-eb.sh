...
function getRoleDescriptor() {
    aws iam list-roles | jq -r '.Roles[] | select(.RoleName == "'"$1"'")'
}
...
function createAppEnvironmentIfNotExists() {
    ...
    getEnvironmentStatus "$NAME" "$ENVNAME"
    if [ ! -z "$ENV_STATUS" ] ; then
        echo "Current environment status is $ENV_STATUS"
    fi
    local TERMINATED_STATUS="Terminated"
    if [ -z "$ENV_DESCRIPTOR" ] || [ "$ENV_STATUS" == "$TERMINATED_STATUS" ] ; then
            echo "INFO: Creating environment '$ENVNAME' for application '$NAME' using instance profile '$INSTANCE_PROFILE' and stack '$STACK'"
            local OPTION_SETTINGS='[{"Namespace":"aws:autoscaling:launchconfiguration","OptionName":"IamInstanceProfile","Value":"'
            OPTION_SETTINGS+="$INSTANCE_PROFILE"
            OPTION_SETTINGS+='"}]'
            echo "Option settings are: '$OPTION_SETTINGS'"
